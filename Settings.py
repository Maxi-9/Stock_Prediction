...
# predict.py Options:
PrintResults = (
    True  # Options: [True, False]     | Should print results of the Prediction
)
PredictExcelDir = (
    "Results/"  # Options: [File Path]       | Sets the directory to output Excel files
)
...
# test.py Options(These settings will purely change the testing program):
...
TrainStocks = [
    ...
    "DLTR",
    "REGN",
]  # Options: [List of Stocks]  | Trains using the set
ModelPath = (
    "Models/Saved/"  # Options: [File directory]  | Sets the model save directory
)


# Excel Options (For test.py and predict.py)
